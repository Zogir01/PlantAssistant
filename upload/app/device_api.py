...
class DeviceAPI:
    ...
    def _update_place_config(self, place_id, v):
        ...
        if "hum_threshold" in v:                config.set(place_id, "hum_threshold",         value=int(v["hum_threshold"]))
        if "hum_target" in v:                   config.set(place_id, "hum_target",            value=int(v["hum_target"]))
        if "min_watering_time" in v:            config.set(place_id, "min_watering_time",     value=s_to_ms(int(v["min_watering_time"])))
        if "max_watering_time" in v:            config.set(place_id, "max_watering_time",     value=s_to_ms(int(v["max_watering_time"])))
        if "wait_for_valve_time" in v:          config.set(place_id, "wait_for_valve_time",   value=min_to_ms(int(v["wait_for_valve_time"])))
        if "post_watering_delay" in v:          config.set(place_id, "post_watering_delay",   value=min_to_ms(int(v["post_watering_delay"])))
        if "measurement_interval" in v:         config.set(place_id, "measurement_interval",  value=min_to_ms(int(v["measurement_interval"])))
        if "min_adc" in v:                      config.set(place_id, "min_adc",               value=int(v["min_adc"]))
        if "max_adc" in v:                      config.set(place_id, "max_adc",               value=int(v["max_adc"]))
        if "sample_count" in v:                 config.set(place_id, "sample_count",          value=int(v["sample_count"]))
        if "sample_interval" in v:              config.set(place_id, "sample_interval",       value=int(v["sample_interval"]))
        config.save()
